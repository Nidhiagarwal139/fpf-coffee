...
const TOTAL_FRAMES = 40; // Adapted to the 40 frames found
...
export default function HeroCanvasAnimation() {
        ...
        const loadImages = async () => {
            const imagePromises = Array.from({ length: TOTAL_FRAMES }, (_, i) => {
                return new Promise<HTMLImageElement>((resolve, reject) => {
                    const img = new Image();
                    // Using jpg as per available assets
                    ...
                    img.onerror = (e) => {
                        ...
                        // Resolve with a placeholder or previous image if possible to avoid breaking everything
                        resolve(img);
                    };
                });
            });
    ...
    useEffect(() => {
        ...
        const renderFrame = () => {
            try {
                ...
                const imgIndex = Math.max(0, Math.min(currentFrame, TOTAL_FRAMES - 1));
                ...
                if (img && img.complete && img.naturalHeight !== 0) {
                    ...
                    // Cover fit might look better for background?
                    // Using cover fit instead of contain for immersive background
                    const scaleCover = Math.max(
                        canvas.width / img.width,
                        canvas.height / img.height
                    );

                    const x = (canvas.width - img.width * scaleCover) / 2;
                    const y = (canvas.height - img.height * scaleCover) / 2;

                    ctx.clearRect(0, 0, canvas.width, canvas.height);
                    ctx.drawImage(img, x, y, img.width * scaleCover, img.height * scaleCover);
                }
            } catch (e) {
    ...
    const section3Opacity = useTransform(smoothProgress, [0.6, 0.65, 0.8, 0.85], [1, 0, 0, 0]); // Adjusted overlap
        ...
        <div ref={containerRef} className="relative h-[500vh]">
            ...
            <div className="sticky top-0 h-screen w-full overflow-hidden">
                ...
                <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                    ...
                    >
                        <h1 className="text-7xl md:text-9xl font-['Playfair_Display'] font-bold text-amber-50 mb-4 tracking-tight drop-shadow-lg">
                            FPF Coffee
                        </h1>
                        <p className="text-xl md:text-2xl text-amber-100/80 font-['Inter'] font-light">
                            Coffee Wale Bhaiya
                        </p>
                    </motion.div>
                    ...
                    <motion.div
                        ...
                        className="text-left px-8 md:px-16 max-w-2xl absolute left-0"
                    >
                    ...
                    <motion.div
                        ...
                        className="text-right px-8 md:px-16 max-w-2xl absolute right-0"
                    >
                        <h2 className="text-5xl md:text-7xl font-['Playfair_Display'] font-semibold text-amber-50 mb-3 drop-shadow-lg">
                            Anti-Gravity Flavor
                        </h2>
                    ...
                    <motion.div
                        ...
                        className="text-center px-4"
                    >
                        <h2 className="text-6xl md:text-8xl font-['Playfair_Display'] font-bold text-amber-50 mb-6 drop-shadow-lg">
                            Discover Your Blend
                        </h2>
                        <motion.button
